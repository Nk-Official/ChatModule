...
extension AttachmentActionSheet{
    ...
    func presentLocationView(){
        let locationViewController = LocationPickerViewController.initiatefromStoryboard(.main)
        if let navigationController  = viewController as? UINavigationController{
            navigationController.navigationBar.isHidden = false
            navigationController.navigationBar.prefersLargeTitles = false
            let searchC = UISearchController(searchResultsController: nil)
            locationViewController.navigationItem.searchController = searchC
            locationViewController.navigationItem.hidesSearchBarWhenScrolling = false
            locationViewController.delegate = locationPickerDelegate
